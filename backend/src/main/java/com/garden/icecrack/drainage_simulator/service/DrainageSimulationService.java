...
public class DrainageSimulationService {
    ...
    private final Executor drainageExecutor;

    public DrainageSimulationService(SimulationResultRepository simulationResultRepository,
                                     PavementRepository pavementRepository,
                                     ApplicationEventPublisher eventPublisher,
                                     ObjectMapper objectMapper,
                                     DrainageProperties props,
                                     @Qualifier("drainageExecutor") Executor drainageExecutor) {
        this.simulationResultRepository = simulationResultRepository;
        this.pavementRepository = pavementRepository;
        this.eventPublisher = eventPublisher;
        this.objectMapper = objectMapper;
        this.props = props;
        this.drainageExecutor = drainageExecutor;
    }

    public CompletableFuture<SimulationResultDTO> runSimulationAsync(SimulationRequestDTO request) {
        return CompletableFuture.supplyAsync(() -> runSimulation(request), drainageExecutor);
    }
    ...
    public SimulationResultDTO runSimulation(SimulationRequestDTO request) {
        ...
        SweResult swe = solveShallowWaterEquations(
                areaLength, areaWidth, slopeAngle, basePermeability, request);

        String timeSeriesJson;
        String gridDataJson;
        try {
            timeSeriesJson = objectMapper.writeValueAsString(swe.timeSeriesList);
            gridDataJson = objectMapper.writeValueAsString(swe.grid);
        } catch (Exception e) {
            timeSeriesJson = "[]";
            gridDataJson = "[]";
        }

        boolean alertTriggered = swe.recessionTimeSec > props.getRecessionAlertThresholdSec();
        String alertMessage = null;
        if (alertTriggered) {
            alertMessage = "Recession time exceeded " + (long) props.getRecessionAlertThresholdSec() + " seconds: " + (long) swe.recessionTimeSec + "s";
        }

        double infiltrationRate = basePermeability * (1 + swe.crackWidthMm / props.getCrackInfiltrationWidthFactor()) * (1 + props.getCrackInfiltrationStepFactor() * swe.stepFrequency);

        SimulationResult result = new SimulationResult();
        result.setPavement(pavement);
        result.setSimTime(LocalDateTime.now());
        result.setInitialWaterDepth(swe.initialDepthM);
        result.setRecessionTimeSec(swe.recessionTimeSec);
        result.setPeakWaterDepth(swe.peakWaterDepth);
        result.setDrainageRate(swe.drainageRate);
        result.setInfiltrationRate(infiltrationRate);
        result.setSurfaceRunoffRate(swe.surfaceRunoffRate);
        ...
        eventPublisher.publishEvent(new SimulationCompletedEvent(
                ...
                swe.recessionTimeSec,
                swe.peakWaterDepth * 1000.0,
                swe.drainageRate,
                infiltrationRate,
                swe.surfaceRunoffRate
        ));
    ...
    public SimulationResultDTO runCustomSimulation(SimulationRequestDTO request,
                                                     double areaLength, double areaWidth,
                                                     double slopeAngle, double basePermeability) {
        SweResult swe = solveShallowWaterEquations(
                areaLength, areaWidth, slopeAngle, basePermeability, request);
        ...
        if (request.getPavementId() != null) {
            dto.setPavementId(request.getPavementId());
        }
        dto.setInitialWaterDepth(swe.initialDepthM);
        dto.setRecessionTimeSec(swe.recessionTimeSec);
        dto.setPeakWaterDepth(swe.peakWaterDepth);
        dto.setDrainageRate(swe.drainageRate);
        dto.setInfiltrationRate(infiltrationRate);
        dto.setSurfaceRunoffRate(swe.surfaceRunoffRate);
        dto.setTimeSeries(timeSeriesJson);
        dto.setGridData(gridDataJson);
        dto.setAlertTriggered(alertTriggered);
        dto.setAlertMessage(alertMessage);
        return dto;
    }

    private static class SweResult {
        double[][] grid;
        double initialDepthM;
        double peakWaterDepth;
        double recessionTimeSec;
        double drainageRate;
        double surfaceRunoffRate;
        double crackWidthMm;
        double stepFrequency;
        List<Map<String, Object>> timeSeriesList;
    }

    private SweResult solveShallowWaterEquations(double areaLength, double areaWidth,
                                                   double slopeAngle, double basePermeability,
        ...
        long seed = 42L;
        if (request.getPavementId() != null) {
            seed = request.getPavementId().getMostSignificantBits() ^ request.getPavementId().getLeastSignificantBits();
        }
        ...
        SweResult result = new SweResult();
        result.grid = grid;
        result.initialDepthM = initialDepthM;
        result.peakWaterDepth = peakWaterDepth;
        result.recessionTimeSec = recessionTimeSec;
        result.drainageRate = drainageRate;
        result.surfaceRunoffRate = surfaceRunoffRate;
        result.crackWidthMm = crackWidthMm;
        result.stepFrequency = stepFrequency;
        result.timeSeriesList = timeSeriesList;
        return result;
    }
